...
# 1 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/include/umf_mem_init_memoryspace.h" 1
 ...
 void umfdl_mem_init_memoryspace
(
    NumericType *Numeric
) ;
...
# 1 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/include/umf_mem_alloc_head_block.h" 1
 ...
 long umfdl_mem_alloc_head_block
(
    NumericType *Numeric,
    long nunits
) ;
...
# 1 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/include/umf_mem_alloc_tail_block.h" 1
 ...
 long umfdl_mem_alloc_tail_block
(
    NumericType *Numeric,
    long nunits
) ;
...
# 1 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/include/umf_mem_free_tail_block.h" 1
 ...
 void umfdl_mem_free_tail_block
(
    NumericType *Numeric,
    long i
) ;
 ...
 **Rpi, nempty_row, dense_row_threshold, empty_elements, rpi, rpx ;
 ...
 return ((0)) ;
    ...
    umfdl_mem_init_memoryspace (Numeric) ;
    ;
...
# 354 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/umf_kernel_init.c"
 ...
 if (!amd_l_valid (n_row, n_col, Ap, Ai))
 {
...
# 470 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/umf_kernel_init.c"
 ...
 for (k = n1 ; k < nn - nempty ; k++)
 {
    ...
    rpi = umfdl_mem_alloc_tail_block (Numeric, (((((sizeof (long *) * (n_row+1))) + (sizeof (Unit)) - 1) / (sizeof (Unit))))) ;
    rpx = umfdl_mem_alloc_tail_block (Numeric, (((((sizeof (double *) * (n_row+1))) + (sizeof (Unit)) - 1) / (sizeof (Unit))))) ;
    if (!rpi || !rpx)
    {
 ...
 size = (((((sizeof (long) * (lnz))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) + (((((sizeof (double) * (lnz))) + (sizeof (Unit)) - 1) / (sizeof (Unit))))
      + (((((sizeof (long) * (unz))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) + (((((sizeof (double) * (unz))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ;
 p = umfdl_mem_alloc_head_block (Numeric, size) ;
 ;
 if (!p)
 {
 ...
 if (esize > 0)
 {
     ...
     E [e] = umfdl_mem_alloc_element (Numeric, esize, 1, &Rows, &Cols, &C,
  &size, &ep) ;
     if (E [e] <= 0)
     {
 ...
 for (k = n1 ; k < n_row - nempty_row ; k++)
 {
  ...
  e++ ;
  ;
  E [e] = umfdl_mem_alloc_element (Numeric, 1, rdeg, &Rows, &Cols,
      &C, &size, &ep) ;
  if (E [e] <= 0)
  {
 ...
 {
     pivot_value = D [k] ;
     umfdl_scale (llen, pivot_value, Lval) ;
 }
 ...
 if (k < n_col - nempty_col)
 {
  ...
  Unit *pp ;
  pp = Memory + E [e] ;
  { { { ; ; ; ep = (Element *) pp ; pp += (((((sizeof (Element) * (1))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ; Cols = (long *) pp ; } ; ncols = ep->ncols ; Rows = Cols + ncols ; } ; nrows = ep->nrows ; pp += (((((sizeof (long) * (ncols + nrows))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ; C = (double *) pp ; } ;
  ;
  ;
  ;
     }
...
# 853 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/umf_kernel_init.c"
 ...
 if (clen != esize)
 {
    ...
    umfdl_mem_free_tail_block (Numeric, rpi) ;
    umfdl_mem_free_tail_block (Numeric, rpx) ;
...
# 970 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/umf_kernel_init.c"
 ...
 Wp [i] = (-1) ;
    ...
    (void) umfdl_tuple_lengths (Numeric, Work, &unused) ;
    if (!umfdl_build_tuples (Numeric, Work))
    {
