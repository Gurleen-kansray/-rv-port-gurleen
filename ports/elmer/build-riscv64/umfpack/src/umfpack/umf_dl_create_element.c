...
static void copy_column (long len, double *X, double *Y)
{
    long i ;
#pragma ivdep
    for (i = 0 ; i < len ; i++)
...
# 214 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/umf_create_element.c"
    needunits = 0 ;
    got_memory = (0) ;
    eloc = umfdl_mem_alloc_element (Numeric, fnrows, fncols, &Rows, &Cols, &C,
 &needunits, &ep) ;
 ...
 ;
 if (!umfdl_get_memory (Numeric, Work, needunits, r2, c2, do_Fcpos))
 {
...
# 437 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/umf_create_element.c"
  ...
  t2 = umfdl_mem_alloc_tail_block (Numeric, needunits) ;
  if (!t2)
  {
      ...
      ;
      return (umfdl_get_memory (Numeric, Work, 0, r2, c2,do_Fcpos));
  }
  Col_tuples [col] = t2 ;
  tp2 = (Tuple *) (Memory + t2) ;
  if (t1)
   ...
   *tp2++ = *tp1++ ;
      ...
      umfdl_mem_free_tail_block (Numeric, t1) ;
  }
     }
