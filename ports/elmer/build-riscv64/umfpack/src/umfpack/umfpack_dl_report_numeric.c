...
)
{
 ...
 (void) umfdl_report_vector (n_row, Numeric->Rs, (double *) 0,
     prl, (0), (1)) ;
    }
    else
    {
 ...
 return ((-3)) ;
    ...
    { if (prl >= (4)) { { (void) printf ("\ndiagonal of U: ") ; } ; } } ;
    (void) umfdl_report_vector (n_inner, (double *) Numeric->D, (double *) 0,
 prl, (0), (0)) ;
 ...
 *Lip, newLchain, llen, prl1, pos, lp, p, npiv, n1, *Li ;
 ...
 if ((llen > 0 || deg > 0)
     && (p + (long) (((((sizeof (double) * (deg))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) > Numeric->size))
 {
     return ((0)) ;
 }
 if (llen > 0)
 ...
 *Uip, n_col, ulen, p, newUchain, up, npiv, n1, *Ui ;
 ...
 xp = (double *) (Numeric->Memory + p) ;
 if (deg > 0 && (p + (long) (((((sizeof (double) * (deg))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) > Numeric->size))
 {
     return ((0)) ;
 }
 for (j = 0 ; j < deg ; j++)
