...
)
{
    ...
    { (d_abs) = ((((*d_mantissa) < 0.)) ? -(*d_mantissa) : (*d_mantissa)) ; } ;
...
)
{
 ...
 { (d_mantissa) *= (pow (10.0, d_exponent)) ; } ;
    }
    else
    {
 Ex [0] = d_exponent ;
