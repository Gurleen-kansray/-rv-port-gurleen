...
# 508 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/umf_garbage_collection.c"
     p = psrc + 1 ;
     { { { ; ; ; epsrc = (Element *) p ; p += (((((sizeof (Element) * (1))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ; Cols = (int *) p ; } ; ncols = epsrc->ncols ; Rows = Cols + ncols ; } ; nrows = epsrc->nrows ; p += (((((sizeof (int) * (ncols + nrows))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ; C = (DoubleComplex *) p ; } ;
