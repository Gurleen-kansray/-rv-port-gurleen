...
# 1 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/include/umf_mem_init_memoryspace.h" 1
 ...
 void umfzi_mem_init_memoryspace
(
    NumericType *Numeric
) ;
...
# 1 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/include/umf_mem_alloc_head_block.h" 1
 ...
 int umfzi_mem_alloc_head_block
(
    NumericType *Numeric,
    int nunits
) ;
...
# 1 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/include/umf_mem_alloc_tail_block.h" 1
 ...
 int umfzi_mem_alloc_tail_block
(
    NumericType *Numeric,
    int nunits
) ;
...
# 1 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/include/umf_mem_free_tail_block.h" 1
 ...
 void umfzi_mem_free_tail_block
(
    NumericType *Numeric,
    int i
) ;
 ...
 **Rpi, nempty_row, dense_row_threshold, empty_elements, rpi, rpx ;
 ...
 return ((0)) ;
    ...
    umfzi_mem_init_memoryspace (Numeric) ;
    ;
...
# 470 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/umf_kernel_init.c"
 ...
 for (k = n1 ; k < nn - nempty ; k++)
 {
    ...
    rpi = umfzi_mem_alloc_tail_block (Numeric, (((((sizeof (int *) * (n_row+1))) + (sizeof (Unit)) - 1) / (sizeof (Unit))))) ;
    rpx = umfzi_mem_alloc_tail_block (Numeric, (((((sizeof (DoubleComplex *) * (n_row+1))) + (sizeof (Unit)) - 1) / (sizeof (Unit))))) ;
    if (!rpi || !rpx)
    {
 ...
 size = (((((sizeof (int) * (lnz))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) + (((((sizeof (DoubleComplex) * (lnz))) + (sizeof (Unit)) - 1) / (sizeof (Unit))))
      + (((((sizeof (int) * (unz))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) + (((((sizeof (DoubleComplex) * (unz))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ;
 p = umfzi_mem_alloc_head_block (Numeric, size) ;
 ;
 if (!p)
 {
 ...
 if (esize > 0)
 {
     ...
     E [e] = umfzi_mem_alloc_element (Numeric, esize, 1, &Rows, &Cols, &C,
  &size, &ep) ;
     if (E [e] <= 0)
     {
 ...
 for (k = n1 ; k < n_row - nempty_row ; k++)
 {
  ...
  e++ ;
  ;
  E [e] = umfzi_mem_alloc_element (Numeric, 1, rdeg, &Rows, &Cols,
      &C, &size, &ep) ;
  if (E [e] <= 0)
  {
 ...
 {
     pivot_value = D [k] ;
     umfzi_scale (llen, pivot_value, Lval) ;
 }
 ...
 if (k < n_col - nempty_col)
 {
  ...
  Unit *pp ;
  pp = Memory + E [e] ;
  { { { ; ; ; ep = (Element *) pp ; pp += (((((sizeof (Element) * (1))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ; Cols = (int *) pp ; } ; ncols = ep->ncols ; Rows = Cols + ncols ; } ; nrows = ep->nrows ; pp += (((((sizeof (int) * (ncols + nrows))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ; C = (DoubleComplex *) pp ; } ;
  ;
  ;
  ;
     }
...
# 853 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/umf_kernel_init.c"
 ...
 if (clen != esize)
 {
    ...
    umfzi_mem_free_tail_block (Numeric, rpi) ;
    umfzi_mem_free_tail_block (Numeric, rpx) ;
...
# 970 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/umf_kernel_init.c"
 ...
 Wp [i] = (-1) ;
    ...
    (void) umfzi_tuple_lengths (Numeric, Work, &unused) ;
    if (!umfzi_build_tuples (Numeric, Work))
    {
