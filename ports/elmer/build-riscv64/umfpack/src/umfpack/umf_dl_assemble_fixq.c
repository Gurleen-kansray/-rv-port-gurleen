...
# 420 "/home/acer/risc-v-hpc/ports/elmer/umfpack/src/umfpack/umf_assemble.c"
 ...
 { { { ; ; ; ep = (Element *) p ; p += (((((sizeof (Element) * (1))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ; Cols = (long *) p ; } ; ncols = ep->ncols ; Rows = Cols + ncols ; } ; nrows = ep->nrows ; p += (((((sizeof (long) * (ncols + nrows))) + (sizeof (Unit)) - 1) / (sizeof (Unit)))) ; S = (double *) p ; } ;
 nrowsleft = ep->nrowsleft ;
 ncolsleft = ep->ncolsleft ;
 next = ep->next ;
 ep->next = (-1) ;
...
#pragma ivdep
   ...
   S += nrows ;
     ...
     umfdl_mem_free_tail_block (Numeric, E [e]) ;
     E [e] = 0 ;
